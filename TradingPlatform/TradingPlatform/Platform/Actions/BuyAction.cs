...
namespace TradingPlatform.Platform.Actions
{
    public class BuyAction : IAction
    {
        ...
        public bool ParseParameters(string parameters)
        {
            ...
            if (values.Length != 2)
            {
                MessageUtil.ShowMessage("Invalid Parameter for Buy stocks, Buy(share,quantity)");
                return false;
            }
            try
            {
            ...
            {
                MessageUtil.ShowMessage("Invalid Parameter for Buy stocks, quantity too big or too small, Buy(share,quantity) - quantity must be integer > 0");
                return false;
            }
            catch (Exception e)
            {
                MessageUtil.ShowMessage("Invalid Parameter for Buy stocks, Buy(share,quantity) - quantity must be integer > 0");
                return false;
            }
            share = values[0].Trim();
            return true;
